...
fn get_rank_compute_status_inner(
    ...
    let result_db_continuity = (|source_path: &str,
        ...
        let actual_dates = (|db_path: &Path,
         ...
         -> Result<Vec<String>, String> {
            ...
            let db_path_str = db_path
                ...
                .ok_or_else(|| format!("{file_name} 路径不是有效 UTF-8"))?;
            let conn =
                Connection::open(db_path_str).map_err(|e| format!("打开 {file_name} 失败: {e}"))?;
            let table_exists = conn
                .query_row(
                    "SELECT COUNT(*) FROM information_schema.tables WHERE table_name = ?",
                    [table_name],
            ...
            Ok(trade_dates)
        })(&result_db, "scoring_result.db", "score_summary")?;
    ...
    })(source_path, &result_db_range)?;
    let cyq_db_range = query_trade_date_range(&cyq_db, "cyq.db", "cyq_snapshot")?;
    let (similarity_rank_db, similarity_active_config) = (|db_path: &Path| -> Result<
        (
            RankComputeDbRange,
            Option<super::trigger_similarity::ranking::StrategyTriggerSimilarityActiveConfig>,
        ),
    ...
    > {
        ...
        let conn = Connection::open(db_path_str)
            .map_err(|e| format!("打开 scoring_result.db 失败: {e}"))?;
        let table_exists = conn
        .query_row(
    ...
    let cyq_factor = (|db_path: &Path| -> Result<Option<u64>, String> {
        if !db_path.exists() {
            return Ok(None);
        }
    ...
    let (cyq_chen_warmup_days, cyq_chen_bucket_pct) =
        (|db_path: &Path| -> Result<(Option<u64>, Option<f64>), String> {
            if !db_path.exists() {
                return Ok((None, None));
            }
            ...
            let conn =
                Connection::open(db_path_str).map_err(|e| format!("打开 cyq_chen.db 失败: {e}"))?;
            let table_exists = conn
        .query_row(
    ...
    let suggested_start_date = match (
        ...
        (_, Some(source_max), Some(result_max)) if result_max < source_max => {
            (|db_path: &Path, after_trade_date: &str| -> Result<Option<String>, String> {
                if !db_path.exists() {
                    return Ok(None);
                }
                ...
                let conn = Connection::open(db_path_str)
                    .map_err(|e| format!("打开 stock_data.db 失败: {e}"))?;
                let table_exists = conn
        .query_row(
    ...
    Ok(RankComputeStatus {
        ...
        strategy_path: (|source_path: &str, strategy_path: Option<&str>| -> String {
...
pub fn run_ranking_score_calculation(
    ...
    Ok(RankComputeRunResult {
        ...
        timings: (|profile: &ScoringRunProfile| -> Vec<RankComputeTimingItem> {
            vec![
...
pub fn run_ranking_tiebreak_fill(
    ...
    Ok(RankComputeRunResult {
        ...
        end_date: None,
        elapsed_ms: started_at.elapsed().as_millis() as u64,
        timings: (|profile: &RankTiebreakProfile| -> Vec<RankComputeTimingItem> {
